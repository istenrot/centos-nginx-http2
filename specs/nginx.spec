...
Dynamic pagespeed module for nginx.
...
./configure %{COMMON_CONFIGURE_ARGS} \
    ...
    --with-openssl-opt="%{openssl_config_options}" \
    --build="%{cust_build_id}" \
    --with-debug
make %{?_smp_mflags}
...
%{__mv} %{_builddir}/%{name}-%{version}/objs/ngx_pagespeed.so \
    %{_builddir}/%{name}-%{version}/objs/ngx_pagespeed-debug.so
./configure %{COMMON_CONFIGURE_ARGS} \
    --with-openssl-opt="%{openssl_config_options}" \
    --build="%{cust_build_id}" \
    --with-cc-opt="%{WITH_CC_OPT}"
...
%{__install} -m755 %{_builddir}/%{name}-%{version}/objs/ngx_pagespeed-debug.so \
    $RPM_BUILD_ROOT%{_libdir}/nginx/modules/ngx_pagespeed-debug.so
...
%{_libdir}/nginx/modules/ngx_pagespeed-debug.so
